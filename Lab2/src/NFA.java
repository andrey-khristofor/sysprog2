...
public class NFA {
    ...
    // transition functions direct from file, with brackets / whitespace / other
    // characters
    ...
    Stack<ArrayList<Integer>> endStack = new Stack<ArrayList<Integer>>();
    ...
    public void acceptableStates() {
        ...
        for (int i = 0; i < this.states.length; ++i) {
            ...
            for (int j = 0; j < this.transFStates.size(); ++j) {
                ...
                if (tempCurrent.current == tempStateNum) {
    ...
    public void getEndStates() {
        ...
        for (int j = 0; j < this.alphabet.length; ++j) {
            ...
            // use stack bc combinedEndStates array keeps changing and can't add/remove in
            // loop
            ...
            while (!miniStack.empty()) {
                ...
                if (this.epsilons.containsKey(t)) {
                    ...
                    for (int x : epsilons.get(t).get(this.alphabet[j]))
                        miniStack.push(x);
                }
            }
                        ...
                        if (!(oneEndStates == null)) {
                            ...
                            for (int a = 0; a < oneEndStates.size(); ++a) {
                                if (this.epsilons.containsKey(oneEndStates.get(a))) {
                                    combinedEndStates = combineArrays(
                                            this.epsilons.get(oneEndStates.get(a)).get(this.alphabet[i]),
                                            combinedEndStates);
                                }
                            }
                        }
    ...
    public String formatFunctions(ArrayList<Integer> startStates, String letter, ArrayList<Integer> endStates) {
        ...
        if (endStatesString.equals("{"))
            return returnString + "EM";
    ...
    public ArrayList<Integer> combineArrays(ArrayList<Integer> smallerSet, ArrayList<Integer> allCombined) {
        ...
        for (int k = 0; k < smallerSet.size(); ++k) {
            if (!allCombined.contains(smallerSet.get(k)))
                allCombined.add(smallerSet.get(k));
        }
        return allCombined;
    }
